...
function updateTypeIndex() {
  ...
  const acts = [
    {
      type: 'append',
      path: resolve(_cwd, 'src/dto/index.ts'),
      template: `export * from './I{{pascalCase moduleName}}.dto'`,
    },
    { type: 'removeEmptyLines', path: resolve(_cwd, 'src/dto/index.ts') },
    {
      type: 'append',
      path: resolve(_cwd, 'src/vo/index.ts'),
      template: `export * from './I{{pascalCase moduleName}}.vo'`,
    },
    { type: 'removeEmptyLines', path: resolve(_cwd, 'src/vo/index.ts') },
  ]
  return acts
}
...
export function type(options) {
  ...
  const res = {
    ...
    actions: (data) => {
      ...
      const __templatePath = join(templatePath, './types/crud')

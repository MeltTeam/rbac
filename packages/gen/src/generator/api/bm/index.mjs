...
function createExceptionCode() {
  ...
  const acts = [
    {
      type: 'modify',
      path: resolve(_cwd, 'src/common/exceptions/exception-code.ts'),
    ...
    { type: 'collectFormat', path: resolve(_cwd, 'src/common/exceptions/exception-code.ts'), packageJsonName: '@apps/api' },
  ]
  return acts
}
/**
...
function updateBmIndex(data, _basePath) {
  ...
  const acts = [
    {
      type: 'append',
      path: resolve(_cwd, 'src/modules/index.ts'),
    ...
    { type: 'removeEmptyLines', path: resolve(_cwd, 'src/modules/index.ts') },
    {
      type: 'modify',
      path: resolve(_cwd, 'src/boot/boot.module.ts'),
    ...
    {
      type: 'modify',
      path: resolve(_cwd, 'src/common/constants/SwaggerTags.ts'),
    ...
    { type: 'collectFormat', path: resolve(_cwd, 'src/modules/index.ts'), packageJsonName: '@apps/api' },
    { type: 'collectFormat', path: resolve(_cwd, 'src/boot/boot.module.ts'), packageJsonName: '@apps/api' },
    { type: 'collectFormat', path: resolve(_cwd, 'src/common/constants/SwaggerTags.ts'), packageJsonName: '@apps/api' },
  ]
  return acts
}
...
export function bm(options) {
  ...
  const res = {
    ...
    actions: (data) => {
      ...
      const __templatePath = join(templatePath, './api/crud')
      const typeTemplatePath = join(templatePath, './types/crud')
      // 树相关的文件
      const TREE_FILES = [

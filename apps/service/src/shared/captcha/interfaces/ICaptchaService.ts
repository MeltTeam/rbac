...
export interface ICaptchaService {
   ...
   * @param type 验证码类型
   */
  delSvgCaptcha: (token: string, type: CaptchaType) => Promise<boolean>
   ...
   * @param type 验证码类型
   */
  verifySvgCaptcha: (token: string, text: string, type: CaptchaType) => Promise<boolean>
   ...
   * @param type 验证码类型(用于redis同一个邮箱不同功能的key)
   */
  delEmailCaptcha: (email: string, type: CaptchaType) => Promise<boolean>
   ...
   * @param type 验证码类型(用于redis同一个邮箱不同功能的key)
   */
  verifyEmailCaptcha: (email: string, code: string, type: CaptchaType) => Promise<boolean>
}

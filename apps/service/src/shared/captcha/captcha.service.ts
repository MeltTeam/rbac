...
export class CaptchaService extends LikeCache2Module implements ICaptchaService {
  ...
  async generateSvgCaptcha(type: CaptchaType, svgCaptchaConfig?: ConfigObject): Promise<ISvgCaptchaVO> {
    ...
    this.set(key, text, 5 * 60 * 1000).then(
      () => this.logger.debug(`${this.generateSvgCaptcha.name}:${key}:${text}`, CaptchaService.name),
      (error) => {
  ...
  async generateEmailCaptcha(options: IGenerateEmailCaptchaOptions) {
    ...
    this.logger.debug(`${this.generateEmailCaptcha.name}:${key}:${code}`, CaptchaService.name)

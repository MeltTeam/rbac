...
export async function createMysqlDatabase(options: ICreateMysqlDatabaseOptions) {
  ...
  try {
    ...
    pool.on('connection', (connection) => {
      logger.info(`${connection.threadId} 新建连接`, loggerContext)
      connection.on('error', (error) => logger.error(`${connection.threadId} 错误: ${error.message}`, loggerContext))
      connection.on('connect', () => logger.info(`${connection.threadId} 连接成功`, loggerContext))
      connection.on('end', () => logger.info(`${connection.threadId} 连接已关闭`, loggerContext))
    })
    pool.on('acquire', (connection) => logger.info(`${connection.threadId} 获取连接成功`, loggerContext))
    pool.on('enqueue', () => logger.info(`连接中...`, loggerContext))
    pool.on('release', async (connection) => {
      logger.info(` ${connection.threadId} 手动释放连接中`, loggerContext)
      connection.destroy()
    })
    ...
    logger.info(`创建数据库 ${database} 成功`, loggerContext)
    const [nowMsg] = await connection.query(`SELECT NOW()`)
    logger.info(`数据库时间:${nowMsg[0]['NOW()']}`, loggerContext)
    connection.release()
  } catch (e) {
